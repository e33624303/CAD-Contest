...
void GDSFILE::clstrm()
{
    ...
    {
		const int shift_bit = 11;
		const unsigned block_size = 1u << shift_bit;
		int pad_zero = Ptr;
		if (Ptr & (block_size - 1)) { pad_zero = ((Ptr >> shift_bit) + 1) << shift_bit; }
        for(i = Ptr; i < pad_zero; i++) Buffer[i] = 0;

        if (write(Fd, Buffer, pad_zero) < 0) errorCnt++;
    }
