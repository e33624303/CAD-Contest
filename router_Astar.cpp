...
bool operator < (Grid_node a, Grid_node b) {
    return a.grid_index.first < b.grid_index.first;
}
void print_coordinate (Grid_node node, GridMap &G, MainInput &M) {
    if (M.LayerSet[node.Layer].Orientation == 1) {
        cout << "( " << G.GridMapSet[node.Layer].track[node.grid_index.first].coord << ", " << G.GridMapSet[node.Layer].track[node.grid_index.first].gridpoint[node.grid_index.second].coord << " )" << endl;
    }
    else {
        cout << "( " << G.GridMapSet[node.Layer].track[node.grid_index.first].gridpoint[node.grid_index.second].coord << ", " << G.GridMapSet[node.Layer].track[node.grid_index.first].coord << " )" << endl;
    }
}
pair<int, int> get_coordinate (Grid_node node, GridMap &G, MainInput &M) {
    pair<int, int> coord;
    if (M.LayerSet[node.Layer].Orientation == 1) {
        coord.first = G.GridMapSet[node.Layer].track[node.grid_index.first].coord;
        coord.second = G.GridMapSet[node.Layer].track[node.grid_index.first].gridpoint[node.grid_index.second].coord;
    }
    else {
        coord.first =  G.GridMapSet[node.Layer].track[node.grid_index.first].gridpoint[node.grid_index.second].coord;
        coord.second = G.GridMapSet[node.Layer].track[node.grid_index.first].coord;
    }
    return coord;
}
bool change_layer (Grid_node origin, Grid_node& target, int t_layer, GridMap &G, MainInput &M) {
    if (M.LayerSet[t_layer].Orientation == M.LayerSet[origin.Layer].Orientation)
        return 0;
    bool find_x = 0, find_y = 0;
    for (int i = 0; i < G.GridMapSet[t_layer].track.size(); i++)
        if (G.GridMapSet[t_layer].track[i].coord == G.GridMapSet[origin.Layer].track[origin.grid_index.first].gridpoint[origin.grid_index.second].coord) {
            target.grid_index.first = i;
            find_x = 1;
            break;
        }
    for (int i = 0; i < G.GridMapSet[t_layer].track[target.grid_index.first].gridpoint.size(); i++)
        if (G.GridMapSet[origin.Layer].track[origin.grid_index.first].coord == G.GridMapSet[t_layer].track[target.grid_index.first].gridpoint[i].coord) {
            target.grid_index.second = i;
            find_y = 1;
            break;
        }
    target.Layer = t_layer;
    if (find_x == 1 && find_y == 1 && G.GridMapSet[target.Layer].track[target.grid_index.first].gridpoint[target.grid_index.second].type == 0) return 1;
    else return 0;
}
int cal_manhattan (Grid_node a, Grid_node b, GridMap &G, MainInput &M) {
    int sum = 0;
    if (M.LayerSet[a.Layer].Orientation == M.LayerSet[b.Layer].Orientation) {
        int abs_tmp = G.GridMapSet[a.Layer].track[a.grid_index.first].coord - G.GridMapSet[b.Layer].track[b.grid_index.first].coord;
        if (abs_tmp < 0) abs_tmp = -abs_tmp;
        sum += abs_tmp;
        abs_tmp = G.GridMapSet[a.Layer].track[a.grid_index.first].gridpoint[a.grid_index.second].coord- G.GridMapSet[b.Layer].track[b.grid_index.first].gridpoint[b.grid_index.second].coord;
        if (abs_tmp < 0) abs_tmp = -abs_tmp;
        sum += abs_tmp;
    }
    else {
        int abs_tmp = G.GridMapSet[a.Layer].track[a.grid_index.first].coord - G.GridMapSet[b.Layer].track[b.grid_index.first].gridpoint[b.grid_index.second].coord;
        if (abs_tmp < 0) abs_tmp = -abs_tmp;
            sum += abs_tmp;
            abs_tmp = G.GridMapSet[a.Layer].track[a.grid_index.first].gridpoint[a.grid_index.second].coord -  G.GridMapSet[b.Layer].track[b.grid_index.first].coord;
            if (abs_tmp < 0) abs_tmp = -abs_tmp;
            sum += abs_tmp;
    }
    return sum;
...
int cal_real_manhattan (Grid_node a, int b , int c){
    int sum = 0 ;
    sum += abs(a.grid_index.first - b);
    sum += abs(a.grid_index.second - c);
    return sum;
}
            ...
            for (int i1 = 0; i1 < 2; i1++) {
                ...
                if (invalid_start) {
                    ...
                    for(int i=0 ; i<B.PinBitSet.size() ; i++){
                        ...
                        //cout << "start[ " << i <<" ] = ( " << start_pin.first << ", " << start_pin.second << " )" << endl;
                        int layer = pintmp.LayerNum;
                        int orientation = M.LayerSet[layer].Orientation;
                        Grid_node node_tmp;
                        node_tmp.Layer = layer;
            ...
            while (!find_the_solution && !time_out) {
                ...
                if (((float)obstacle_occur_node.size())/((float)GN_origin.size()) > 0.65 && !partial_solution && !try_next && !zero_seg) { // BIG OBSTACLE OCCUR
                    ...
                    if (T.up_down == 0) {
                        ...
                        while (base_num == 0) {
                            tg2.grid_index.second++;
                            base_num = cal_manhattan(tg1, tg2, G, M);
                        }
...
bool bus_router_func (All_route &all, GridMap &G, MainInput &M) {
    cout << "Bus Router start\n";
    all.types.resize(M.BusSet.size());
    all.path.resize(M.BusSet.size());
    BUSES_block B_block;
    Preserve_space P;
    //all all;
    ...
    for(int i = 0 ; i < M.BusSet.size() ; i++){
        //if (i != 2) continue;
        cout << "Running Bus " << i << endl;
        bool ans_flag = 0;
        ans_flag = one_bus_router(all, G, M.BusSet[i], M, B_block, P, i );
        BUSES_block_deactivate (B_block, i);
        BUSES_preserve_deactivate (P, i);
        cout << "Finish Running Bus " << i << endl;
    }
    cout << "Bus Router end\n";
